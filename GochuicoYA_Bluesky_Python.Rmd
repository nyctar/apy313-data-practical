...
The Alpindale Bluesky posts repository is imported. Initially, I sampled a subset of 100,000 posts from the larger 2 million-post dataset while doing the tests for efficiency. But, for the final run, I decided to use the whole dataset.
...
I used the VADER Sentiment Lexicon to compute sentiment scores for each post. VADER is well-suited for social media text, as it takes into account text variations commonly used in online communication.

The scores are from -1 to 1 (more negative - more positive).

This will serve as the sentiment variable (DV - response).

```{r}
# Load vaderSentiment from Python
vader <- import("vaderSentiment.vaderSentiment", convert = TRUE)
analyzer <- vader$SentimentIntensityAnalyzer()

# Apply VADER sentiment analysis to each text
tb <- tb %>%
  mutate(
    sentiment = map_dbl(text, ~ analyzer$polarity_scores(.x)$compound)
  )

# Check a few results
tb %>% select(text, sentiment, time_of_day) %>% head(10)
```
